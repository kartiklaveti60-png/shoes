    ...
    <>
      ...
      <div
        ...
        className={`fixed top-0 left-0 pointer-events-none z-[99999] rounded-full transition-all duration-150 ease-out ${
          isInputHovered
            ? 'w-1 h-5 bg-[#D52122] rounded-none opacity-90 scale-100'
            : isClicked
            ? 'w-2 h-2 bg-[#D52122] scale-150 shadow-[0_0_12px_#D52122]'
            : isHovered
            ? 'w-3 h-3 bg-[#D52122] scale-110 shadow-[0_0_10px_rgba(213,33,34,0.7)]'
            : 'w-2 h-2 bg-[#1A1008] opacity-90'
        }`}
      ...
      <div
        ...
        className={`fixed top-0 left-0 pointer-events-none z-[99998] flex items-center justify-center rounded-full transition-all duration-300 ease-out border ${
          isInputHovered
            ...
            : isClicked
            ? 'w-10 h-10 border-[#D52122] bg-[#D52122]/20 scale-90 shadow-[0_0_20px_rgba(213,33,34,0.35)]'
            : isHovered
            ? 'w-14 h-14 border-[#D52122]/80 bg-[#D52122]/10 scale-100 shadow-[0_0_25px_rgba(213,33,34,0.2)] backdrop-blur-[2px]'
            : 'w-8 h-8 border-[#1A1008]/20 bg-transparent scale-100 shadow-sm'
        }`}
        style={{ willChange: 'transform' }}
      >
        {cursorText && (
          <span className="text-[9px] font-extrabold tracking-widest uppercase text-[#D52122] px-1 select-none animate-pulse">
            {cursorText}
          </span>
        )}
      </div>
